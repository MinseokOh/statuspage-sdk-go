...
// Time represents a custom time type that handles both string and time formats from the API
type Time struct {
	time.Time
}

// UnmarshalJSON handles various time formats from the API
func (t *Time) UnmarshalJSON(b []byte) error {
	...
	// Try to unmarshal as time.Time directly
	return json.Unmarshal(b, &t.Time)
}

// MarshalJSON marshals the time to RFC3339 format
...
// GetLogoURL extracts the URL from a logo field (json.RawMessage)
func GetLogoURL(logoField json.RawMessage) string {
	if len(logoField) == 0 {
		return ""
	}
	...
	// Try to unmarshal as string first
	var s string
	if err := json.Unmarshal(logoField, &s); err == nil {
		return s
	}
	
	// Try to unmarshal as object and get URL
	var obj struct {
		URL            string `json:"url"`
		OriginalURL    string `json:"original_url"`
		NormalURL      string `json:"normal_url"`
	}
	if err := json.Unmarshal(logoField, &obj); err == nil {
		if obj.URL != "" {
			return obj.URL
		}
		if obj.OriginalURL != "" {
...
// GetLogoObject extracts the logo as a structured object if it contains metadata
func GetLogoObject(logoField json.RawMessage) *Logo {
	if len(logoField) == 0 {
		return nil
	}
	
	var logo Logo
	if err := json.Unmarshal(logoField, &logo); err != nil {
		return nil
	}
	return &logo
}

// Logo represents a logo object with URL and metadata from the API
type Logo struct {
...
type Incident struct {
	...
	MonitoringAt                  *Time               `json:"monitoring_at,omitempty"`
	Name                          string              `json:"name,omitempty"`
	PageID                        string              `json:"page_id,omitempty"`
	PostmortemBody                string              `json:"postmortem_body,omitempty"`
	PostmortemBodyLastUpdatedAt   *Time               `json:"postmortem_body_last_updated_at,omitempty"`
	...
	PostmortemPublishedAt         *Time               `json:"postmortem_published_at,omitempty"`
	...
	ScheduledFor                  *Time               `json:"scheduled_for,omitempty"`
	ScheduledRemindPrior          bool                `json:"scheduled_remind_prior,omitempty"`
	ScheduledRemindedAt           *Time               `json:"scheduled_reminded_at,omitempty"`
	ScheduledUntil                *Time               `json:"scheduled_until,omitempty"`
